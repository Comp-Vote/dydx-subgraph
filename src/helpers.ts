...
export function getUser(address: Address): User {
  ...
  if (!user) {
    ...
    user.wrappedTokenVotingPower = BigInt.fromI32(0);
    user.wrappedTokenProposingPower = BigInt.fromI32(0);
    user.numberVotes = 0;
  }
...
export function getRewardsClaimed(userAddress: Address, incentivesModule: String, claimTimestamp: BigInt): RewardsClaimed {
  let userRewardId: RewardsClaimedId = getRewardsClaimedId(userAddress, incentivesModule, claimTimestamp);
...
function getRewardsClaimedId(userAddress: Address, incentivesModule: String, claimTimestamp: BigInt): RewardsClaimedId {
